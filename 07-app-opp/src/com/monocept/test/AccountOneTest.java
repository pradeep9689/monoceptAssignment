...
import com.monocept.model.AccType;
import com.monocept.model.AccountOne;

public class AccountOneTest {

	public static void main(String[] args) {
		AccountOne account[]=new AccountOne[2];
		Scanner scanner=new Scanner(System.in);
		for(int i=0;i<account.length;i++) {
			account[i]=new AccountOne();
...
;			if(accType==1) {
	account[i].setAccType(AccType.Svaing);
}
if(accType==2) {
	account[i].setAccType(AccType.Current);
}
		}
		for(int i=0;i<account.length;i++) {
